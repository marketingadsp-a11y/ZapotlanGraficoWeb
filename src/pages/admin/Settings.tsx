...
import { Save, Image as ImageIcon, Globe, Palette, ShieldCheck, User, Key, Bell, Clock, Facebook, Instagram, Twitter, Youtube, Video, Newspaper, X, Check, RefreshCw, Copy } from 'lucide-react';
...
export default function Settings() {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          setSettings({
            ...
            autoSyncYouTube: data.autoSyncYouTube !== false,
            autoSyncFacebook: data.autoSyncFacebook !== false,
  ...
  const handleSyncYouTubeNow = async () => {
    if (!settings.youtubeUrl) {
      toast.error('Por favor ingresa primero la URL de tu canal de YouTube');
      return;
    }
    setSyncingYt(true);
    const toastId = toast.loading('Sincronizando videos y transmisiones de YouTube...');
    try {
      const result = await syncYouTubeVideosToArticles(settings.youtubeUrl, true);
      if (result.success) {
        if (result.addedCount > 0) {
          toast.success(`¡Sincronización exitosa! Se ${result.addedCount === 1 ? 'creó 1 nuevo artículo' : `crearon ${result.addedCount} nuevos artículos`}.`, { id: toastId });
        } else {
          toast.info(`Canal al día (${result.totalProcessed} videos verificados, 0 nuevos).`, { id: toastId });
        }
      } else {
        toast.error(result.error || 'Error al conectar con YouTube', { id: toastId });
      }
    } catch (err: any) {
  ...
  const handleSyncFacebookNow = async () => {
    if (!settings.facebookUrl) {
      toast.error('Por favor ingresa primero el enlace a tu página de Facebook');
      return;
    }
    setSyncingFb(true);
    const toastId = toast.loading('Sincronizando publicaciones de Facebook...');
    try {
      const result = await syncFacebookPostsToArticles(settings.facebookUrl, true, settings.facebookPageAccessToken);
      if (result.success) {
        if (result.addedCount > 0) {
          toast.success(`¡Sincronización exitosa! Se ${result.addedCount === 1 ? 'publicó 1 nueva nota en Facebook' : `publicaron ${result.addedCount} nuevas notas en Facebook`}.`, { id: toastId });
        } else {
          toast.info('Sección Facebook al día (sin publicaciones nuevas).', { id: toastId });
        }
      } else {
          ...
          >
            <Card className="border-none shadow-sm bg-white rounded-[2.5rem] overflow-hidden">
              ...
              <CardContent className="p-8 space-y-6">
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="space-y-4 md:col-span-2 p-5 rounded-3xl bg-blue-50/40 border border-blue-100">
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Facebook (Página Oficial)</label>
                      <div className="relative">
                        ...
                        <Input 
                          ...
                          className="h-14 pl-12 rounded-2xl border-slate-200 bg-white focus:bg-white text-xs transition-colors"
                          placeholder="https://facebook.com/zapotlan.grafico"
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">
                          Token de Acceso de Página de Facebook (Graph API - Opcional)
                        </label>
                        <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest bg-slate-100 px-2 py-0.5 rounded-full">Para sync masivo</span>
                      </div>
                      <Input 
                        type="password"
                        value={settings.facebookPageAccessToken || ''}
                        onChange={(e) => setSettings(prev => ({ ...prev, facebookPageAccessToken: e.target.value }))}
                        className="h-12 rounded-xl border-slate-200 bg-white focus:bg-white text-xs font-mono transition-colors"
                        placeholder="EAA... (Token de Meta Developers)"
                      />
                      <p className="text-[9px] text-slate-400 font-medium ml-2">
                        Si cuentas con un token de Meta Graph API para tu página, ingrésalo aquí para sincronizar automáticamente todo el historial de publicaciones como artículos.
                      </p>
                    </div>

                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-3 border-t border-blue-200/50">
                      <div className="space-y-0.5">
                        <p className="text-xs font-black uppercase tracking-wider text-slate-900 flex items-center gap-1.5">
                          <Check className="h-3.5 w-3.5 text-blue-600" />
                          Auto-sincronizar publicaciones en sección "Facebook"
                        </p>
                        <p className="text-[10px] text-slate-500 font-medium leading-relaxed">
                          Cada publicación de Facebook se crea como artículo y se publica **exclusivamente** en la sección Facebook sin mezclarse con las noticias generales.
                        </p>
                      </div>
                      <div className="flex items-center gap-3 shrink-0">
                        <button
                          type="button"
                          onClick={() => setSettings(prev => ({ ...prev, autoSyncFacebook: prev.autoSyncFacebook === false ? true : false }))}
                          className={cn(
                            "relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none",
                            settings.autoSyncFacebook !== false ? "bg-[#1877F2]" : "bg-slate-200"
                          )}
                          title="Activar o desactivar la sincronización de Facebook"
                        >
                          <span
                            className={cn(
                              "pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out",
                              settings.autoSyncFacebook !== false ? "translate-x-5" : "translate-x-0"
                            )}
                          />
                        </button>
                        <Button
                          type="button"
                          variant="outline"
                          disabled={syncingFb || !settings.facebookUrl}
                          onClick={handleSyncFacebookNow}
                          className="h-10 rounded-xl border-blue-200 bg-white hover:bg-blue-50 text-[#1877F2] font-black uppercase text-[10px] tracking-wider px-4 shadow-sm"
                        >
                          <RefreshCw className={cn("mr-1.5 h-3.5 w-3.5", syncingFb && "animate-spin")} />
                          {syncingFb ? 'Sincronizando...' : 'Sincronizar ahora'}
                        </Button>
                      </div>
                    </div>

                    {/* Webhook URL Helper */}
                    <div className="p-3 bg-white/80 rounded-2xl border border-blue-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                      <div className="space-y-0.5 min-w-0">
                        <p className="text-[10px] font-black uppercase tracking-wider text-slate-700">Webhook de Conexión en Tiempo Real</p>
                      ...
                      <Button
                        ...
                        variant="ghost"
                        onClick={() => {
                          const url = `${window.location.origin}/api/webhook-facebook`;
                          navigator.clipboard.writeText(url);
                          toast.success('URL del Webhook copiada al portapapeles');
                        }}
                        className="h-8 rounded-lg text-slate-600 hover:text-[#1877F2] hover:bg-blue-50 text-[10px] font-black uppercase tracking-wider shrink-0"
                      >
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <Instagram className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-[#E1306C]" />
                      <Input 
                  ...
                  <div className="space-y-4 md:col-span-2 p-5 rounded-3xl bg-slate-50 border border-slate-100">
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">YouTube (Canal Oficial)</label>
                      <div className="relative">
                        ...
                        <Input 
                          ...
                          className="h-14 pl-12 rounded-2xl border-slate-200 bg-white focus:bg-white text-xs transition-colors"
                          placeholder="https://youtube.com/@ZapotlánGraficoMX"
                        />
                      </div>
                    </div>

                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-3 border-t border-slate-200/70">
                      <div className="space-y-0.5">
                        <p className="text-xs font-black uppercase tracking-wider text-slate-900 flex items-center gap-1.5">
                          <Check className="h-3.5 w-3.5 text-green-600" />
                          Auto-publicar videos de YouTube como artículos
                        </p>
                        <p className="text-[10px] text-slate-500 font-medium leading-relaxed">
                          Al activarlo, cada nuevo video o transmisión en vivo de YouTube se publica automáticamente como artículo en **Noticias**, con reproductor y notas.
                        </p>
                      </div>
                      <div className="flex items-center gap-3 shrink-0">
                        <button
                          type="button"
                          onClick={() => setSettings(prev => ({ ...prev, autoSyncYouTube: prev.autoSyncYouTube === false ? true : false }))}
                          className={cn(
                            "relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none",
                            settings.autoSyncYouTube !== false ? "bg-red-600" : "bg-slate-200"
                          )}
                          title="Activar o desactivar la publicación automática"
                        >
                          <span
                            className={cn(
                              "pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out",
                              settings.autoSyncYouTube !== false ? "translate-x-5" : "translate-x-0"
                            )}
                          />
                        </button>
                        <Button
                          type="button"
                          variant="outline"
                          disabled={syncingYt || !settings.youtubeUrl}
                          onClick={handleSyncYouTubeNow}
                          className="h-10 rounded-xl border-red-200 bg-white hover:bg-red-50 text-red-600 font-black uppercase text-[10px] tracking-wider px-4 shadow-sm"
                        >
                          <RefreshCw className={cn("mr-1.5 h-3.5 w-3.5", syncingYt && "animate-spin")} />
                          {syncingYt ? 'Sincronizando...' : 'Sincronizar ahora'}
                        </Button>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <Video className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-900" />
                      <Input 
